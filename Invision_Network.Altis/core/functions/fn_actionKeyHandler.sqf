...
if (count (nearestObjects [player,["UserTexture1m_F"],5] select {!isNil {_x getVariable ["Ermen_RealisticFarmingName",nil]}}) > 0) exitWith {
    if (life_action_gathering) exitWith {};
    private _handle = [] spawn Ermen_fnc_RealisticFarmingGather;
